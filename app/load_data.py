    ...
    async with SessionLocal() as session:
        for csv_file in csv_files:
            ...
            for row in reader:
                ...
                try:
                    device_timestamp = datetime.strptime(timestamp_str, "%d-%m-%Y %H:%M")
                except Exception:
                    continue
                ...
                try:
                    glucose_record = GlucoseRecord(
                        ...
                        record_type=int(row.get("Aufzeichnungstyp", 0)) 
                            if row.get("Aufzeichnungstyp") else None,
                        glucose_value=int(row.get("Glukosewert-Verlauf mg/dL", 0)) 
                            if row.get("Glukosewert-Verlauf mg/dL") else None,
                        glucose_scan=int(row.get("Glukose-Scan mg/dL", 0)) 
                            if row.get("Glukose-Scan mg/dL") else None,
                    )
                    session.add(glucose_record)
                    records.append(glucose_record)
                except Exception as e:
                    msg = f"❌ Failed to create GlucoseRecord for row in {csv_file.name}"
